...
class medoo
{
    ...
    public function query($query)
    {
        ...
        if (__SQL_DEBUG__) {
            echo "查询语句<br />" . $query . "<br /><br />";
        }
        \vd($query,'<font color="red">!!!! query</font>');
        error_log("\n>>query\n" . $query . "\n\n\n", 3, __LOG_FILE__);
        self::$SQLS[] = $query;
        // $rs = $this->pdo->query($query);
        $rs = $this->pdo->query($query,PDO::FETCH_ASSOC);
        // \vd($rs, 'medoo rs');
        return $rs;
    }
    ...
    public function insert($table, $datas)
    {
        ...
        foreach ($datas as $data) {
            ...
            if (__SQL_DEBUG__) {
                echo "插入语句<br />" . $sql . "<br /><br />";
            }
            \vd($sql,'SQL:'.$this->prefix . $table);
    ...
    public function update($table, $data, $where = null)
    {
        ...
        $sql = 'UPDATE `' . $this->prefix . $table . '` SET ' . implode(', ', $fields) . $this->where_clause($where);
        if (__SQL_DEBUG__) {
            echo "更新语句<br />" . $sql . "<br /><br />";
        }
        \vd($sql);
        error_log("\n>>update\n" . $sql . "\n\n\n", 3, __LOG_FILE__);
    ...
    {
        $query = 'DELETE FROM `' . $this->prefix . $table . '`' . $this->where_clause($where);
        if (__SQL_DEBUG__) {
            echo "删除语句<br />" . $query . "<br /><br />";
        }
        error_log("\n>>delete\n" . $query . "\n\n\n", 3, __LOG_FILE__);
